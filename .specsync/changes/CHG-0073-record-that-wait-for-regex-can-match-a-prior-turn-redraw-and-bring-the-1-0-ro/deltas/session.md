...
9. `prompt_detected` is advisory metadata and **never** gates whether a call returns.
   ...
   hatch, provided the pattern cannot already be sitting in the child's visible history.
...
- **A reused `--wait-for-regex` pattern can match a reprint of an earlier answer.** `Echo#repaint?`
  ...
  motion as the real new answer. Unique-per-turn sentinels wait through the reprint. Poll
  `child_busy` or a destination file rather than trusting `matched` alone against a TUI that
  repaints scrollback.
