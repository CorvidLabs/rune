...
| `unknown_flag_error` | internal method | Rejects a flag-shaped token before the first operand. A flag `run` owns, spelled correctly but given a space-separated value, gets the inline-value message instead of "Unknown option". |
| `VALUE_FLAGS` | constant | The flags that take a value, derived from `FLAG_PATTERNS` so it cannot drift from the parser. |
| `INLINE_VALUE_ERROR` | constant | The message for a correctly spelled flag whose value was space-separated. |
